...
function requireBabelPlugin(pluginName) {
  ...
  const pluginPath = require.resolve(`${pluginName}/package`);
...
module.exports = {
  ...
  included(appOrAddon) {
    ...
    if (!this._registeredWithBabel && this._isCoverageEnabled()) {
      const checker = new VersionChecker(this.parent).for('ember-cli-babel', 'npm');
      ...
      if (checker.satisfies('>= 6.0.0')) {
        ...
        concat(
          ...
          this._findInRepoAddons().map(addon => addon.addon || addon)
        )
          .filter(Boolean)
          .map(getPlugins)
  ...
  contentFor(type) {
    if (type === 'test-body-footer' && this._isCoverageEnabled()) {
      const templatePath = path.join(__dirname, 'lib', 'templates', 'test-body-footer.html');
      const template = fs.readFileSync(templatePath).toString();

      const fileLookupStrings = JSON.stringify(
        Object.keys(this.fileLookup).map(file => file.replace(EXT_RE, ''))
      );

      return template.replace('{%ENTRIES%}', fileLookupStrings);
    }
  ...
  _getIncludesForAppDirectory() {
    ...
    const prefix = this.parent.isEmberCLIAddon()
      ? 'dummy'
      : this.parent.name();
  ...
  _getIncludesForAddonDirectory() {
    const addon = this._findCoveredAddon();
  ...
  _getIncludesForInRepoAddonDirectories() {
    return this._findInRepoAddons().reduce((acc, { addon, addonPath }) => {
      const addonDir = addonPath && addonPath.length
        ? path.join(this.project.root, addonPath)
        : path.join(this.project.root, 'lib', addon.name);

      const addonAppDir = path.join(addonDir, 'app');
      const addonAddonDir = path.join(addonDir, 'addon');
  ...
  _getIncludesForDir(dir, prefix) {
    if (fs.existsSync(dir)) {
      const dirname = path.relative(this.project.root, dir);
      const globs = this.parentRegistry.extensionsForType('js').map((extension) => `**/*.${extension}`);
   ...
   */
  _isCoverageEnabled() {
    let value = process.env[this._getConfig().coverageEnvVar] || false;
   ...
   */
  _parentName() {
    return this.parent.isEmberCLIAddon()
      ? this._findCoveredAddon().name
      : this.parent.name();
  },
  ...
  _findInRepoAddons() {
    if (!this._inRepoAddons) {
      ...
      const inRepoPaths = pkg['ember-addon'] && pkg['ember-addon'].paths;

      this._inRepoAddons = (inRepoPaths || []).map((addonPath) => {
        const name = path.basename(addonPath);
        const addon = this.project.findAddonByName(name);

        return addon ? { addon, addonPath } : false;
      }).filter(Boolean);
    }
